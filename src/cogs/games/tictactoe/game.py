...
class RematchButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        self.view.headline = (
            f"{self.view.p1.display_name} VS {self.view.p2.display_name}"
        )
        self.view.description = f"{self.view.turn.display_name}'s Turn"
...
class Game(discord.ui.View):
    ...
    def turn(self) -> discord.Member:
        ...
        moves = [
            0 for b in self.children if isinstance(b, TicTacToeButton) and b.owner
        ].count(0)
    ...
    def get_moves(self, player: discord.Member | None) -> list[int]:
        ...
        return [
            ...
            if isinstance(b, TicTacToeButton) and b.owner == player
        ]
